results_dir = '/Users/matthewoshaughnessy/Dropbox/class38/';
results_folder = 'selectAlpha__mnist_JOINT_UNCOND_zdim8_alpha1_No100_Ni25_lam0.005_class38';
...
for k = 1:nlatentdim
  ...
  if strcmpi(images,'separate')
    ...
    if export
      saveas(gcf,sprintf('figs/fig_mnist_qual_%d.svg',latent_dim(k)));
      export_fig('-transparent',sprintf('figs/fig_mnist_qual_%d.pdf',latent_dim(k)))
    end
  end
...
if strcmpi(images,'combined')
  set(gcf,'color','white');
  if export
    export_fig('figs/fig_mnist_qual.pdf');
  end
end